...
def split_catchment(wid: str, basin: int, lat: float, lng: float, catchment_poly: Polygon,
    ...
    # ─── Force use of the 61.0 flowdir tile only ───────────────────────────
    forced_tile = "61.0"
    fdir_fname = os.path.join(MERIT_FDIR_DIR, f"flowdir{forced_tile}.tif")
    if VERBOSE:
        print(f"Loading flow direction raster from: {fdir_fname}")
        print(f" using windowed reading mode with bounding_box = {repr(bounding_box)}")

    if not os.path.isfile(fdir_fname):
        raise Exception(f"Could not find flow direction raster: {fdir_fname}")
